...
def main():
    ...
    bucket_name = os.getenv('GCS_BUCKET_NAME', 'th-ai-talent-data')
    
    # Download input data from GCS
    print("Downloading input data from Cloud Storage...")
    data_files = [
        'employees.csv', 'employeeSkill.csv', 'skills.csv', 'positions.csv',
        'departments.csv', 'positionSkill.csv', 'employeeMovement.csv',
        'evaluationRecord.csv'
    ]
    ...
    for file in data_files:
        try:
            download_from_gcs(bucket_name, f'mock_data/{file}', f'mock_data/{file}')
        except Exception as e:
            print(f"Warning: Could not download {file}: {e}")
    
    # Run the original analysis logic
    print("Starting skill promotion analysis...")
    ...
    id_to_canonical_map = normalized_skill_df.set_index('id')['canonical_id'].to_dict()
    canonical_id_to_name_map = normalized_skill_df.drop_duplicates(subset=['canonical_id']).set_index('canonical_id')['canonical_name'].to_dict()
    ...
    emp_df = emp_df[~emp_df['emp_id'].isin(movement_df[movement_df['movement_type'].isin([1, 2])]['employee_id'])]
    ...
    print("Analyzing employee skill gaps...")
    ...
    print("Analyzing department skill gaps...")
    ...
    print("Analyzing rotation opportunities...")
    # --- 3. Employee Rotation ---
    ...
    print("Analyzing promotion readiness...")
    ...
    output_files = [
        'employee_skill_gap_result.json',
        'department_skill_gap_result.json', 
        'rotation_skill_gap_result.json',
        'promotion_analysis_results.json'
    ]
    
    for file in output_files:
        try:
            upload_to_gcs(bucket_name, f'output/{file}', f'output/{file}')
        except Exception as e:
